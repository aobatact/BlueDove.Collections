...
namespace BlueDove.SlimCollections
{
    ...
    public class DictionarySlim<TKey, TValue> : IReadOnlyCollection<KeyValuePair<TKey, TValue>> where TKey : IEquatable<TKey>
    {
        ...
        public DictionarySlim(DictionarySlim<TKey, TValue> values)
        {
            _count = values._count;
            _freeList = values._freeList;
            _buckets = new int[values._buckets.Length];
            values._buckets.CopyTo(_buckets.AsSpan());
            _entries = new Entry[_entries.Length];
            values._entries.CopyTo(_entries.AsSpan());
        }
